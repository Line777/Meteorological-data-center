...
CZHOBTMIND::CZHOBTMIND(conection *conn,CLogFile *logfile)
{
  m_conn=conn;
  m_logfile=logfile;
}   
...
{ 
  ...
  if (isxml==true)
  {
    ...
    GetXMLBuffer(strBuffer,"p",m_zhobtmind.t,10);
    if (strlen(tmp)>0)
    {
  ...
  else 
  {
    ...
    CmdStr.GetValue(0,"obtid",m_zhobtmind.obtid,10);
    CmdStr.GetValue(1,"ddatetime",m_zhobtmind.ddatetime,14);
    char tmp[11];
...
bool CZHOBTMIND::InsertTable();
{
  if (m_stmt.m_state==0)
  {
    ...
    m_stmt.bindin(2,m_zhobtmind.ddatetime,10);
  ...
  if (m_stmt.execute()!=0)
  {
    if (m_stmt.m_cda.rc!=1062)  //内容非法，排除重复记录的情况
    {
