...
char strcolvalue[MAXCOLCOUNT][MAXCOLLEN+1];  // 插入和更新的sqlstatement对象
...
void preparesql()
{
  ...
  for (int ii=0;ii<TABCOLS.m_vallcols.size();ii++)
  {
    ...
    logfile.WriteEx("stmtins.bindin(%d,%s,%d)\n",colseq,TABCOLS.m_vallcols[ii].colname,TABCOLS.m_vallcols[ii].col);
    colseq++;
  }
    
  // 绑定更新sql语句的输入变量
  // 如果入库分数中指定了表数据不需要更新，就不处理update语句了，函数返回
  if (strxmltotable.uptbz!=1)
  {
    return;
  }
  ...
  for (int ii=0;ii<TABCOLS.m_vallcols.size();ii++)
  {
    ...
    // strclovalue数组的使用不是连续的                    
    stmtupt.bindin(colseq,strcolvalue[ii],TABCOLS.m_vallcols[ii].collen);
    logfile.WriteEx("stmtup.bindin(%d,%s,%d)\n",colseq,TABCOLS.m_vallcols[ii].colname,TABCOLS.m_vallcols[ii].col);
    colseq++;
  }
}
